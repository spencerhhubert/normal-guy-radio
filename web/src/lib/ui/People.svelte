<script lang="ts">
	...
	// the list lives on <body>, where no scrolling, masked or clipped ancestor can hide it
...
<span class="people" bind:this={root}>
	<button onclick={toggle} title={people.map((p) => p.name).join(', ')} aria-expanded={open} aria-label="{people.length} listening">
		{#each shown as p (p.name)}<i>{p.emoji}</i>{/each}
		{#if people.length > shown.length}<em>+{people.length - shown.length}</em>{/if}
	</button>
</span>
{#if open}
	<ul use:portal style:top="{at.top}px" style:right="{at.right}px">
...
<style>
	...
	button { display: inline-flex; align-items: center; padding: 0 0 0 6px; background: none; border: 0; color: inherit; font: inherit; }
	i { display: inline-grid; place-items: center; width: 22px; height: 22px; margin-left: -6px; border-radius: 50%; font-style: normal; font-size: 13px; line-height: 1; background: rgba(0, 0, 0, 0.5); box-shadow: 0 0 0 1px rgba(255, 255, 255, 0.18); }
	em { margin-left: 5px; font-style: normal; font-size: 11px; }
	ul { position: fixed; z-index: 9; margin: 0; padding: 6px 0; list-style: none; min-width: 200px; max-height: 60vh; overflow: auto; border-radius: 9px; border: 1px solid #000; background: linear-gradient(180deg, #3b2f27, #2c231d); box-shadow: inset 0 1px 0 rgba(255, 255, 255, 0.12), 0 12px 30px rgba(0, 0, 0, 0.6); color: #e8dcc6; font: 13px/1.3 -apple-system, "Helvetica Neue", Helvetica, Arial, sans-serif; }
	li { display: flex; align-items: center; gap: 9px; padding: 6px 14px; white-space: nowrap; }
	li i { margin: 0; font-size: 13px; }
