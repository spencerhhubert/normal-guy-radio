...
<style>
	.panel {
		display: flex; flex-direction: column; min-height: 0; padding: 18px 20px 20px; border-radius: 22px; color: #e8dcc6;
	...
	.list, .band { --row: 47px; overflow-y: auto; scrollbar-width: none; mask-image: linear-gradient(transparent, #000 var(--top, 0px), #000 calc(100% - var(--bot, 0px)), transparent); -webkit-mask-image: linear-gradient(transparent, #000 var(--top, 0px), #000 calc(100% - var(--bot, 0px)), transparent); }
	.list::-webkit-scrollbar, .band::-webkit-scrollbar { display: none; }
