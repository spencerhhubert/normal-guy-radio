<script lang="ts">
	...
	// picker-wheel look: rows shrink and fade with distance from the middle
	function spin() {
		if (!wheel) return;
		const r = wheel.getBoundingClientRect(), c = r.top + r.height / 2;
		for (const el of wheel.querySelectorAll<HTMLElement>('li')) {
			const b = el.getBoundingClientRect(), k = Math.min(1, Math.abs(b.top + b.height / 2 - c) / (r.height / 2));
			el.style.opacity = String(1 - 0.85 * k * k); el.style.transform = `scale(${1 - 0.2 * k})`;
		}
	}
	$effect(() => { heard.length; requestAnimationFrame(spin); });
...
<section class="panel">
	...
	{#if heard.length}
		...
		<div class="wheel" class:many={heard.length > 5} bind:this={wheel} onscroll={spin}>
			<ul>
...
<style>
	...
	.wheel { --row: 44px; }
	.wheel.many { height: calc(var(--row) * 5); overflow-y: auto; scroll-snap-type: y proximity; scrollbar-width: none; mask-image: linear-gradient(transparent, #000 22%, #000 78%, transparent); -webkit-mask-image: linear-gradient(transparent, #000 22%, #000 78%, transparent); }
	.wheel.many::-webkit-scrollbar { display: none; }
	.wheel.many ul { padding: calc(var(--row) * 2) 0; }
	.wheel.many li { scroll-snap-align: center; transition: opacity 120ms linear, transform 120ms linear; }
	...
	.band { display: grid; grid-template-columns: repeat(auto-fill, minmax(52px, 1fr)); gap: 5px; max-height: 190px; overflow: auto; padding: 2px; scrollbar-width: thin; }
	...
	@media (max-width: 1040px) { .panel { position: static; max-height: none; } }
	@media (max-width: 640px) {
		.panel { padding: 14px 12px 16px; }
		li button { grid-template-columns: 54px 1fr auto; gap: 8px; }
		.call { display: none; }
